...
const OUTPUT_DIR = path.join(__dirname, '..', 'tests', 'output_dehydrate');
...
async function runTest() {
    ...
    server.listen(PORT, async () => {
        ...
        cloner.on('close', (code) => {
            ...
            if (code === 0) {
                console.log('\n--- Verifying Results ---');
                const clonedFilePath = path.join(__dirname, '..', 'tests/output_dehydrate/localhost/clone/index.html');
